...
export default class TalentCard extends React.Component {
  ...
  render() {
    ...
    feedData.videoUrl = feedData.videoUrl ? feedData.videoUrl : "";

    feedData.photoId = feedData.photoId ? feedData.photoId : "";
    return (
      <Card className="talent-card">
        <Card.Content>
          ...
          <div height="250px" className="talent-card img">
            {!profileDisplay ? (
              feedData.videoUrl != null ? (
                <video
                  style={{ width: "90%", height: "auto" }}
                  className="image"
                  controls
                  src={feedData.videoUrl}
                />
              ) : (
        ...
        <Card.Content>
          <Grid columns={4}>
            <Grid.Column>
              {!profileDisplay ? (
                <Icon name="user" size="large" onClick={this.changeStatus} />
              ) : (
        ...
        <Card.Content extra>
          {feedData.skills != null
            ? feedData.skills.map(x => (
                <Button type="button" basic color="blue" key={x}>
                  {x}
                </Button>
              ))
            : null}
